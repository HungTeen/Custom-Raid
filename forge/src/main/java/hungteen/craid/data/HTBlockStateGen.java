...
import hungteen.craid.common.impl.registry.suit.TreeSuits;
...
public abstract class HTBlockStateGen extends BlockStateProvider {
    ...
    protected void woodIntegration(TreeSuits.TreeSuit woodIntegration) {
        ...
        }, () -> CustomRaidAPI.logger().warn("Wood Integration {} Data Gen skipped, because wood planks was banned !", woodIntegration.getRegistryName()));
    ...
    protected <T extends Block> void gen(T block, Consumer<T> consumer) {
        if (this.contains(block)) {
            CustomRaidAPI.logger().warn("Already gen {} before !", key(block));
            return;
        }
    ...
    protected <T extends Block, K extends Block> void gen(T block1, K block2, BiConsumer<T, K> consumer) {
        if (this.contains(block1) || this.contains(block2)) {
            CustomRaidAPI.logger().warn("Already gen block state of {} or {} before !", key(block1), key(block2));
            return;
        }
