package hungteen.craid.common.network;

import hungteen.craid.api.CustomRaidAPI;
import hungteen.craid.common.network.packets.DummyEntityPacket;
import hungteen.craid.common.network.packets.PlaySoundPacket;
import hungteen.craid.common.network.packets.SyncDatapackPacket;
    ...
    public static <T extends HTPlayToServerPayload> BiConsumer<T, CustomPayloadEvent.Context> wrapServerHandler(BiConsumer<T, ServerPacketContext> consumer) {
        return (t, payloadContext) -> {
            ...
            if (player != null) {
                ...
                CustomRaidAPI.logger().debug("Tried to handle server packet payload with no player: {}", t.getClass().getSimpleName());
            }
        };
    }
    ...
    public static <T extends HTPlayToClientPayload> BiConsumer<T, CustomPayloadEvent.Context> wrapClientHandler(BiConsumer<T, ClientPacketContext> consumer) {
        return (t, payloadContext) -> {
            ...
            if (player != null) {
                ...
                CustomRaidAPI.logger().debug("Tried to handle client packet payload with no player: {}", t.getClass().getSimpleName());
            }
        };
    }
